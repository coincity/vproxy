...
app.get('/canales.m3u8', async function (req, res) {
    try {
        ...
        for (const channel of channels) {
            ...
            // Tus filtros de categorías personalizados
            if (name.includes('historia') || name.includes('nat') || name.includes('discovery') || name.includes('odisea')) {
                groupTitle = 'DOCUMENTALES';
            ...
            // Solo se añade a la lista si coincide con tus grupos
            if (groupTitle !== '') {
                output.push(`#EXTINF:-1 tvg-name="${channel.name}" group-title="${groupTitle}" tvg-logo="${channel.logo}" tvg-id="${channel.name}",${channel.name}`);
        ...
        console.log('[vavoo] canales.m3u8 error', error.message);
        res.status(500).send(error.message);
    }
});
